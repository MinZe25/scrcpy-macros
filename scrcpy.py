...
class OverlayWindow(QWidget):
    def __init__(self):
        ...
        # Apply taskbar-like styling to the button
        self.edit_button.setStyleSheet("""
            ...
            QPushButton:hover {
                background-color: rgba(60, 60, 60, 220); /* Slightly more opaque on hover */
        ...
        """)
        self.edit_button.setFixedSize(160, 25)  # Fixed size for consistency
        ...
        if self.scrcpy_window_gw and self.scrcpy_hwnd:  # Ensure we have a valid HWND for scrcpy
            ...
            if should_be_visible:
                ...
                if (self.x() != screen_x or
                    ...
                    # --- Reposition the Edit Mode button to top taskbar position ---
                # Place it at the top right of the overlay window in the window title bar area
                button_margin = 5
                ...
                panel_margin = 10
                # Position panel above the taskbar button
    ...
    def paintEvent(self, event):
        ...
        # Draw a taskbar-like background at the top for the edit button
        if self.isVisible() and not self.edit_mode_active:  # Only when not in edit mode
            taskbar_height = self.edit_button.height() + 6  # Button height plus minimal padding
            taskbar_rect = QRectF(0, 0, self.width(), taskbar_height)
            taskbar_color = QColor(30, 30, 30, 150)  # Dark semi-transparent
            painter.fillRect(taskbar_rect, taskbar_color)

        # Draw a taskbar-like background at the bottom for the edit button
        if self.isVisible() and not self.edit_mode_active:  # Only when not in edit mode
            taskbar_height = self.edit_button.height() + 20  # Button height plus padding
            taskbar_rect = QRectF(0, self.height() - taskbar_height, self.width(), taskbar_height)
            taskbar_color = QColor(30, 30, 30, 120)  # Dark semi-transparent
            painter.fillRect(taskbar_rect, taskbar_color)
        ...
        for p_id, p_data in DRAWING_PRIMITIVES.items():
            ...
            if p_type == 'circle':
                ...
                if key_combo:
                    ...
                    # Calculate font size based on circle radius with much more aggressive scaling
                    # For very small circles, use a much smaller scaling factor
                    if radius < 30:
                        base_font_size = max(int(radius * 0.25), 6)  # Much smaller text for small circles
                    else:
                        base_font_size = min(max(int(radius * 0.35), 8), 24)  # Reduced max size too

                    # Get the key combo length to adjust font size further
                    key_combo_length = len(key_combo)

                    # More aggressive scaling factor for text length
                    if key_combo_length <= 1:  # Single character
                        length_factor = 1.0
                    elif key_combo_length <= 3:  # Very short text
                        length_factor = 0.85
                    elif key_combo_length <= 5:
                        length_factor = 0.7
                    else:  # Longer text gets substantial reduction
                        length_factor = 0.5

                    # Calculate final font size with length adjustment
                    font_size = int(base_font_size * length_factor)
                    font_size = max(font_size, 6)  # Allow smaller minimum size
                    ...
                    # For small circles, use a more compact approach
                    display_text = key_combo  # This will be potentially modified below

                    # For very small circles, prioritize showing just the key part without modifiers
                    if radius < 20 and '+' in key_combo:
                        # Just show the key part after the + for very small circles
                        display_text = key_combo.split('+')[-1]

                    # Determine text width to calculate background size
                    metrics = painter.fontMetrics()
                    text_width = metrics.width(display_text)
                    text_height = metrics.height()

                    # Calculate background size based on text dimensions
                    # For small circles, make background just big enough for text
                    if radius < 30:
                        # Make background just big enough for the text with small padding
                        padding_x = max(4, radius * 0.15)  # Horizontal padding
                        padding_y = max(2, radius * 0.1)  # Vertical padding
                        bg_width = text_width + (padding_x * 2)
                        bg_height = text_height + (padding_y * 2)
                    else:
                        # For larger circles, use more proportional sizing
                        bg_width = max(radius * 1.2, text_width * 1.3)
                        bg_height = max(radius * 0.7, text_height * 1.5)
                    ...
                    # Always use rounded rectangles for better text readability
                    # Semi-transparent black background
                    painter.setPen(Qt.NoPen)
                    painter.setBrush(QColor(0, 0, 0, 180))

                    # Calculate corner radius based on size - smaller for small circles
                    corner_radius = min(bg_height / 3, 8 if radius < 30 else 12)
                    painter.drawRoundedRect(bg_rect, corner_radius, corner_radius)

                    # Add a white border for contrast - thinner for small circles
                    border_width = 1 if radius < 20 else 2
                    painter.setPen(QPen(QColor(255, 255, 255, 200), border_width))
                    painter.setBrush(Qt.NoBrush)
                    painter.drawRoundedRect(bg_rect, corner_radius, corner_radius)

                    # Draw text with bright color and improved contrast
                    # No outline for very small text to keep it crisp and readable
                    if radius < 20:
                        painter.setPen(QPen(QColor(255, 255, 255, 255), 1))  # Thin solid white for small circles
                    else:
                        text_thickness = 2 if font_size >= 16 else 1
                        painter.setPen(QPen(QColor(255, 255, 255, 255), text_thickness))

                    # Use the background rect for text alignment, with small padding
                    text_rect = QRectF(
                        bg_rect.x() + 2,  # Small padding from background edge
                        bg_rect.y() + 2,
                        bg_rect.width() - 4,
                        bg_rect.height() - 4
                    )
                    ...
                    # Check if additional text processing is needed
                    metrics = painter.fontMetrics()
                    text_width = metrics.width(display_text)
                    available_width = bg_rect.width() - 6  # Small safety margin

                    # If text is still too wide, use more aggressive abbreviation
                    if text_width > available_width:
                        # Further abbreviate keys if needed
                        if '+' in display_text and len(display_text.split('+')[-1]) > 1:
                            # For key combinations, just use first letter of the key
                            key_part = display_text.split('+')[-1][0]
                            modifier_part = display_text.split('+')[0]
                            display_text = f"{modifier_part}+{key_part}"
                        elif len(display_text) > 1:
                            # Use just the first character for any other multi-char text
                            display_text = display_text[0]

                        # Re-check if it fits now
                        text_width = metrics.width(display_text)
                        if text_width > available_width and radius < 20:
                            # If still too big for very small circles, use just the first character
                            display_text = display_text[0] if display_text else ""
        ...
        if self.edit_mode_active:
            ...
            painter.drawText(15 + shadow_offset, 210 + shadow_offset,
                             "- Type keys to assign a shortcut (click elsewhere to cancel)")
            ...
            if clicked_primitive_id and clicked_primitive_id in DRAWING_PRIMITIVES:
                ...
                if key_combo:
                    ...
                    if self.scrcpy_hwnd:
                        ...
                        # Parse the key combo based on new format (S,C,A)+Key
                        has_ctrl = 'C' in key_combo and '+' in key_combo
                        has_alt = 'A' in key_combo and '+' in key_combo
                        has_shift = 'S' in key_combo and '+' in key_combo

                        # Extract the key part (after the plus if present, or the whole string if no modifiers)
                        if '+' in key_combo:
    ...
    def keyPressEvent(self, event):
        ...
        if self.edit_mode_active:
            ...
            # Handle key combo assignment for selected primitive
                ...
                is_modifier_key = key in (16777248, 16777249, 16777251)  # Shift, Ctrl, Alt keys

                # Determine if we have modifiers
                has_ctrl = modifiers & Qt.ControlModifier
                has_alt = modifiers & Qt.AltModifier
                has_shift = modifiers & Qt.ShiftModifier

                # If this is a modifier key press with no existing key combo, just return and wait for the next key
                existing_key_combo = DRAWING_PRIMITIVES[self.selected_primitive_id].get('key_combo', '')
                if is_modifier_key and not existing_key_combo:
                    return

                # Get the key name (using shorter names for better text fitting)
                key_name = ""
                if key in (16777216, 16777217, 16777219, 16777220, 16777223):  # Esc, Tab, Backspace, Enter, Delete
                    if key == 16777216:
                        ...
                        key_name = "BS"  # Shorter version of Backspace
                    elif key == 16777220:
                        key_name = "Ent"  # Shorter version of Enter
                    elif key == 16777223:
                        key_name = "Del"  # Shorter version of Delete
                ...
                # Skip processing if only a modifier key is pressed
                if key in (16777248, 16777249, 16777251):  # Shift, Ctrl, Alt keys
                    return

                # Use more compact format for better text fitting in small circles
                modifier_str = ""
                if has_shift or has_ctrl or has_alt:
                    # Build a compact modifier string
                    if has_ctrl:
                        modifier_str += "C"
                    if has_alt:
                        modifier_str += "A"
                ...
                key_combo = f"{modifier_str}{key_name}"

                # Assign the key combo to the primitive
                if key_name:  # Only assign if we have a non-modifier key
    ...
    def toggle_edit_mode(self):
        ...
        if self.edit_mode_active:
            ...
            self.edit_button.setText("Exit Edit Mode")
            self.edit_button.setStyleSheet("""
                QPushButton {
                    background-color: rgba(200, 60, 60, 220); /* Semi-transparent red */
                    color: white;
                    border: 1px solid rgba(240, 100, 100, 220);
                    border-radius: 5px;
                    padding: 5px 10px;
                    font-weight: bold;
                }
                QPushButton:hover {
                    background-color: rgba(220, 80, 80, 240);
                    border: 1px solid rgba(255, 120, 120, 240);
                }
                QPushButton:pressed {
                    background-color: rgba(180, 50, 50, 240);
                }
            """)
        else:
...
if __name__ == "__main__":
    ...
    print("  - Shortcut keys shown as (S,C,A)+Key format for modifiers on circles")
    print("  - Press modifier (Shift, Ctrl, Alt) then another key for 2-key combo")
    print("  - Press non-modifier key for single key combo")
